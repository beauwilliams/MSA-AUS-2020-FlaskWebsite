...
export function GithubSearch(options, renderer) {
    const query = new Subject();
    const pick = new RegExp(options.pick);
    const drop = new RegExp(options.drop);
    const cache = {};
    const results = query.pipe(switchMap(q => (q in cache) ? of({ result: cache[q] }) : // --> respond from cache if query in cache
        ajax.getJSON(`https://api.github.com/search/code?q=${encodeURIComponent(q)}`
    ...
    return renderer.create(ToCSearchBtn, { label: options.label, query: query, results: results });
}
