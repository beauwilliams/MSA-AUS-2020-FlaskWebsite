"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
var rxjs_1 = require("rxjs");
var operators_1 = require("rxjs/operators");
var emission_error_1 = require("../shared/errors/emission-error");
var pipe_1 = require("./pipe");
/**
 *
 * Represents [filter](https://connective.dev/docs/filter) pins.
 *
 */
var Filter = /** @class */ (function (_super) {
    __extends(Filter, _super);
    function Filter(_func) {
        var _this = _super.call(this, (_func.length <= 1) ?
            ([operators_1.filter(function (emission) {
                    try {
                        return _func(emission.value);
                    }
                    catch (error) {
                        throw new emission_error_1.EmissionError(error, emission);
                    }
                })]) :
            ([
                operators_1.mergeMap(function (emission) {
                    return new rxjs_1.Observable(function (subscriber) {
                        _func(emission.value, function (res) {
                            subscriber.next(res);
                            subscriber.complete();
                        }, function (error) {
                            subscriber.error(new emission_error_1.EmissionError(error, emission));
                        }, emission.context);
                    })
                        .pipe(operators_1.filter(function (_) { return !!_; }), operators_1.map(function (_) { return emission; }));
                }),
                operators_1.share()
            ])) || this;
        _this.filter = _func;
        return _this;
    }
    return Filter;
}(pipe_1.Pipe));
exports.Filter = Filter;
/**
 *
 * Creates a [filter](https://connective.dev/docs/filter) pin using given predicate.
 * A filter pin will pass some values through and not others based on given predicate.
 * [Checkout the docs](https://connective.dev/docs/filter) for examples and further information.
 *
 * @param filter
 *
 */
function filter(filter) { return new Filter(filter); }
exports.filter = filter;
/**
 *
 * Creates a [filter](https://connective.dev/docs/filter) that never allows any value through.
 *
 */
function block() { return new Filter(function () { return false; }); }
exports.block = block;
exports.default = filter;
//# sourceMappingURL=filter.js.map